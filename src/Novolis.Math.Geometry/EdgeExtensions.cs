...
public static class EdgeExtensions
{
    ...
    {    
        ...
        if (System.Math.Abs(edge.A.Z) <= TOLERANCE && System.Math.Abs(edge.B.Z) <= TOLERANCE &&
            System.Math.Abs(other.A.Z) <= TOLERANCE && System.Math.Abs(other.B.Z) <= TOLERANCE)
        {
            var p = edge.A;
            var p2 = edge.B;
            var q = other.A;
            ...
            if (System.Math.Abs(denominator) < TOLERANCE) // Lines are parallel.
                return false;
        ...
        else
        {
            ...
            if (System.Math.Abs(Vector3.Dot(Vector3.Cross(da, db), dc)) > float.Epsilon) // lines are not coplanar
                return false;
            ...
            if (System.Math.Abs(dp - 1f) < float.Epsilon || System.Math.Abs(dp + 1f) < float.Epsilon) // lines are parallel
                return false;
            ...
            float t2;
            if (System.Math.Abs(dp - 1f) < float.Epsilon || System.Math.Abs(dp + 1f) < float.Epsilon)
